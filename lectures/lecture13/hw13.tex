...
\begin{frame}[fragile]{HW 13 - turn in one week from today in Canvas}
Turn in the following question as a single .py file onto canvas. Use comments to clearly indicate which question you are working on. Your filename should end as \_py2.py if you use Python2 and \_py3.py if you use Python3.

I've attached stock data for AAPL, MSFT, and PG as CSV files.
\begin{enumerate}
\item Load the three CSV files as DataFrames in Python using Pandas. (You may need to use na\_values='null')
\item Create a new column in each DataFrame named \textit{Range}, which is the absolute difference between the \textit{High} and \textit{Low}.
\item Save the new DataFrames as CSV files. (AAPL\_range.csv, MSFT\_range.csv, and PG\_range.csv)
\item Print the summary statistics of the \textit{Range} column for each DataFrame by using .describe()
\item Plot the Close prices for all stocks between 2008-2009 on the same figure. 
\end{enumerate}
