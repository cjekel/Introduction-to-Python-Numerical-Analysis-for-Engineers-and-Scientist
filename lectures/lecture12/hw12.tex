...
\begin{frame}[fragile]{HW 12 - turn in one week from today in Canvas}
...
Recall the data generated on the last HW:
...
\begin{frame}{HW 12 - turn in one week from today in Canvas}
Use five fold cross validation (CV) to select which regression model is the most appropriate (linear model, quadratic polynomial degree=2, polynomial degree=10). 
\begin{enumerate}
\item Calculate the the $R^2$ value (on the test set) for each model during each CV iteration. 
\item Print the average $R^2$ value for each model. 
\item Select which model had the highest average $R^2$ as the best model for this data. Print which model was the best.
\end{enumerate}
\end{frame}
